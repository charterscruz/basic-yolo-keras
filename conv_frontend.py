...
# class ONLY to extract features
class YoloExtractor(object):
    def __init__(self, backend,
        ...
        # TODO: Commented this part of the code, because I only want to extract feactures here
        ...
        self.model = Model(input_image, features)
...
# Class to be used only to train the convolutional LSTM layers
class YoloConvLSTM(object):
    def __init__(self, backend,
        ...
        new_bias = np.random.normal(size=weights[1].shape) / (self.input_size * self.input_size)
    ...
    def train(self, train_imgs,  # the list of images to train the model
        ...
        generator_config = {
            ...
            'GRID_H': self.input_size,
            'GRID_W': self.input_size,
            'BOX': self.nb_box,
            'LABELS': self.labels,
    ...
    def predict(self, image):
        ...
        image = np.reshape(image, (-1, 10, 13, 13, 1024))
...
# Class to be used only to deploy the conv LSTM
class deployYoloConvLSTM(object):
    def __init__(self, backend,
                 input_size,
                 input_time_horizon,
                 labels,
...
class YoloImgConvLSTM(object):
    def __init__(self, backend,
        ...
        if backend == 'Inception3':
            ...
            self.feature_extractor = FullYoloFeature(self.input_size)
        # elif backend == 'Full Yolo Time Dist':
        #     self.feature_extractor = FullYoloFeatureTimeDist(self.input_size)
        elif backend == 'Tiny Yolo':
            self.feature_extractor = TinyYoloFeature(self.input_size)
        elif backend == 'VGG16':
            self.feature_extractor = VGG16Feature(self.input_size)
