...
class YOLO_timeDist(object):
    def __init__(self, backend,
        ...
        old_yolo = YOLO_old(input_size=input_size,
                            labels=labels,
                            max_box_per_image=max_box_per_image,
                            anchors=anchors)
        old_yolo.load_weights('models/full_yolo_retrain_boat.h5')

        for _, old_layer in enumerate(old_yolo.model.layers[1].layers):
            # print old_layer.name
            for _, new_layer in enumerate(self.feature_extractor.feature_extractor.layers):
                if new_layer.name[:16] == 'time_distributed':
                    if new_layer.layer.name == old_layer.name:
                        # print('same layer in time distributed')
                        # print old_layer.name
                        # print new_layer.layer.name
                        new_layer.layer.weights[:] = old_layer.weights[:]
                        new_layer.layer.trainable = False
...
class YOLO_old(object):
    ...
    def train(self, train_imgs,  # the list of images to train the model
        ...
        tensorboard = TensorBoard(
            log_dir=os.path.expanduser('~/logs/convlstm_yolo' + str(time_horizon) + '_' + str(time_stride)),
            histogram_freq=0,
            # write_batch_performance=True,
            write_graph=True,
            write_images=False)
