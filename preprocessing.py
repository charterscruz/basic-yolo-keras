...
#
#
# def parse_annotation_features(ann_dir, img_dir, labels=[]):
#     all_imgs = []
#     seen_labels = {}
#
#     for ann in sorted(os.listdir(ann_dir)):
...
# Class for batch generator for features "images"
# class BatchGeneratorFeatures(Sequence):
#     def __init__(self, images,
#                  config,
#                  shuffle=True,
#                  jitter=True,
...
#     def load_image(self, i):
#         # return cv2.imread(self.images[i]['filename'])
#         print self.images[i]['filename']
#         return np.load(self.images[i]['filename'])
#
#     def __getitem__(self, idx):
#         l_bound = idx * self.config['BATCH_SIZE']
#         r_bound = (idx + 1) * self.config['BATCH_SIZE']
...
#         instance_count = 0
#
#         x_batch = np.zeros((r_bound - l_bound, self.config['IMAGE_H'], self.config['IMAGE_W'], 1024))  # input images
...
# Class for batch generator for sequences of features "images"
...
class BatchGeneratorFeatureSequences(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for train_instance in self.images[l_bound:r_bound]:
            # print(self.images[l_bound])
            # print(self.images[l_bound + instance_count + self.config['TIME_HORIZON']])
            # get the index of the gt to load
            gt_filename = str(int(os.path.split(train_instance['filename'])[1][:-4])
                                  + self.config['TIME_HORIZON'] -1) \
                                  + '.npy'
            name_to_search = os.path.split(train_instance['filename'])[0] + '/' + gt_filename

            gt_instance = 0
            for img_itr in self.images:
                if img_itr['filename'] == name_to_search:
                    gt_instance = img_itr
            ...
            # except:
            #     variable = 'error on aug_sequences'
